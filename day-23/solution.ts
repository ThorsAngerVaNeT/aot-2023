...
export type Connect4<Game extends Connect4Game, ColumnIndex extends number> = {
  board: UpdateBoard<Game, ColumnIndex>;
  state: GetState<
    UpdateBoard<Game, ColumnIndex>,
    Game["state"] extends Connect4State ? Game["state"] : never
  >;
};
...
type UpdateBoard<
  ...
  Board extends Connect4Cell[][] = [],
> = Game["board"] extends [...infer Rows extends Connect4Cell[][], infer Row extends Connect4Cell[]]
  ? Row[ColumnIndex extends keyof Row ? ColumnIndex : never] extends Connect4EmptyCell
  ? UpdateBoard<
    { board: Rows; state: Game["state"] },
    -1,
    [UpdateRow<Row, ColumnIndex, Game["state"]>, ...Board]
  >
...
type IsItDraw<Game extends Connect4Board> = Connect4EmptyCell extends Game[number][number]
  ? false
  : true;
...
type CheckCollection<
  Collection extends Connect4Cell[],
  Chip extends Connect4Chips,
> = Collection extends [
...
> = Rows extends [infer F extends unknown[], ...infer R extends unknown[][]]
  ? GetColumn<R, Index, [...Result, F[Index]]>
  : Result;

type CheckDiagonals<
  Diagonals extends Connect4Cell[][],
  Chip extends Connect4Chips,
> = Diagonals extends [infer D extends Connect4Cell[], ...infer R extends Connect4Cell[][]]
  ? D[number] extends Chip
  ? true
  : CheckDiagonals<R, Chip>
  : false;
type GetDiagonals<Board extends Connect4Cell[][]> = [
  ...GetRightDiagonals<Board>,
  ...GetLeftDiagonals<Board>,
];
